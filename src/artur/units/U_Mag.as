	...
	public class U_Mag extends MagDoll
	{
		...
		public function frees():void {
			...
			for (var i:int = 0; i < parts.length; i++) {
				if (Sprite(parts[i]).numChildren > 1) {
					var item:U_MagItemsCash = U_MagItemsCash(Sprite(parts[i]).getChildAt(1));
					item.frees();
				}
			}
		...
		private function setItem(type:int, frame:int):void {
			if (Sprite(parts[type]).numChildren > 1) {
				var item:U_MagItemsCash = U_MagItemsCash(Sprite(parts[type]).getChildAt(1));
				if(item.getId != frame) {
					item.frees();
					Sprite(parts[type]).addChild(U_MagItemsCash.getItem(type, frame));
				}
				return;
			}
		   Sprite(parts[type]).addChild(U_MagItemsCash.getItem(type, frame));
