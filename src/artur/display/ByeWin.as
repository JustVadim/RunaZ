package artur.display 
{
	...
	public class ByeWin extends dialogBye
	{
		...
		public function ByeWin() 
		{
			...
			// btnAddMoney= new BaseButton(10);
				//btnAddMoney.addEventListener(MouseEvent.CLICK, onBtn);
		...
		public function init(text1:String = "",text2:String="",priceGold:int=0,priceSilver:int=0,index:int=NaN,byeType:int=0,itemType:int = NaN):void
		{
		...
		public function byeItem(indexPrice:int):void
		{
			 ...
			 var obj:Object = { ns:WinCastle.currSlotClick, hn:index, p:indexPrice,it:itemType };
			 var data:DataExchange = new DataExchange();
		...
		private function getRess2(e:DataExchangeEvent):void
		{
			...
			if (obj.error == null)
			{
				...
				else
				{
				 ...
				 UserStaticData.hero.units[int(WinCastle.currSlotClick)].it[itemType] = Maker.clone(UserStaticData.magazin_items[UserStaticData.hero.units[int(WinCastle.currSlotClick)].t ][itemType][index] ) ;
